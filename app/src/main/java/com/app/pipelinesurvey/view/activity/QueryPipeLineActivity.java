...
/**
 * Created by hairun on 2018-07-10 14:25.
 * 查询编辑管类、修改管类
 */

public class QueryPipeLineActivity extends BaseActivity implements View.OnClickListener, AdapterView.OnItemSelectedListener, IQueryPipeLineView, IDrawPipeLineView {

    private TextView tvTitle;//标题
    private TextView tvSubmit;//提交
    private Button btnEdit;//设置可编辑
    ...
    private EditText edtStartPoint;//管线点号
    private EditText edtEndPoint;//连接点号
    ...
    private BaseFieldLInfos m_baseInfo = null; //线对象
    //    private boolean isEdit = true; //默认开启
    private Button btnDelLine; //删除按钮
    private int m_smId;
    private DatasetVector m_dsV;
    private Recordset m_reSet; //线数据集
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_query_pipe_line);
        initView();
        initValue();
    }

    private void initValue() {
//        SharedPreferences _sharedPreferences = getSharedPreferences("data", MODE_PRIVATE);
...
//                LogUtills.e("获取的线对象错误....");
            ...
            m_reSet.setGeometry(new GeoLine(m_point2Ds));
            if (!m_reSet.update()) {
                LogUtills.i("get start point update line fail");
            }
            //更改线的长度
            m_reSet.setString("pipeLength", String.format("%.2f", m_reSet.getDouble("SmLength")));
            if (!m_reSet.update()) {
                LogUtills.i("get start point update line length fail");
            }
            ...
            m_reSet.setGeometry(new GeoLine(m_point2Ds));
            if (!m_reSet.update()) {
                LogUtills.i("get end point update line fail");
            }
            //更改线的长度
            m_reSet.setString("pipeLength", String.format("%.2f", m_reSet.getDouble("SmLength")));
            if (!m_reSet.update()) {
                LogUtills.i("get end point update line length fail");
            }
        }
    ...
    private void initView() {
        ...
        spEmbeddedWay = $(R.id.spEmbeddedWay);
        spEmbeddedWay.setOnItemSelectedListener(this);
        spTextrure = $(R.id.spTextrure);
        spVolatage = $(R.id.spVoltage);
        spPressure = $(R.id.spPressure);
        spState = $(R.id.spState);
        layoutPipeSize = $(R.id.layoutPipeSize);
        layoutSection = $(R.id.layoutSection);
        layoutDLLDPanel = $(R.id.layoutDLLDPanel);
        edtBurialDifference = $(R.id.edtBurialDifference);
        edtStartBurialDepth = $(R.id.edtStartBurialDepth);
        edtStartBurialDepth.addTextChangedListener(m_watcher);
        edtEndBurialDepth = $(R.id.edtEndBurialDepth);
        edtEndBurialDepth.addTextChangedListener(m_watcher);
        edtLineRemark = $(R.id.edtLineRemark);
        imgbtnExchange = $(R.id.imgbtnExchange);
        imgbtnExchange.setOnClickListener(this);
        edtStartPoint = $(R.id.edtStartPoint);
        edtEndPoint = $(R.id.edtEndPoint);
        imgvLineRemark = $(R.id.imgvLineRemark);
        imgvLineRemark.setOnClickListener(this);
        layoutLineMark = $(R.id.layoutLineMark);
        imgvOwnershipUnit = $(R.id.imgvOwnershipUnit);
        imgvOwnershipUnit.setOnClickListener(this);
        edtOwnershipUnit = $(R.id.edtOwnershipUnit);
        edtOwnershipUnit.setOnClickListener(this);
        edtPipeSize = $(R.id.edtPipeSize);
        edtSectionWidth = $(R.id.edtSectionWidth);
        edtSectionHeight = $(R.id.edtSectionHeight);
        edtPipeLength = $(R.id.edtPipeLength);
        edtHoleCount = $(R.id.edtHoleCount);
        edtUsedHoleCount = $(R.id.edtUsedHoleCount);
        edtAmount = $(R.id.edtAmount);
        edtAperture = $(R.id.edtAperture);
        edtAmount = $(R.id.edtAmount);
        edtRow = $(R.id.edtRow);
        edtCol = $(R.id.edtCol);
        edtPuzzle = $(R.id.edtPuzzle);
        imgvPipeSize = $(R.id.imgvPipeSize);
        imgvPipeSize.setOnClickListener(this);
        layoutPipeSizeEdt = $(R.id.layoutPipeSizeEdt);
        btnDelLine = $(R.id.btnRemove);
        btnDelLine.setOnClickListener(this);
        btnSave = $(R.id.btnSave);
        btnSave.setOnClickListener(this);
        btnEdit = (Button) $(R.id.btnEdit);
        btnEdit.setOnClickListener(this);
        btnGetStartPoint = (Button) $(R.id.btnGetStartPoint);
        btnGetEndPoint = (Button) $(R.id.btnGetEndPoint);
...
//                setViewEdit();
            ...
            case R.id.btnSave:
                ...
                if (!_result) {
                    ToastUtil.show(this.getBaseContext(), "保存线数据失败", 2);
                    return;
                } else {
                    ToastUtil.show(this.getBaseContext(), "保存线数据成功", 2);
                    finish();
                }
//                DataHandlerObserver.ins().setMapActionType(MAPACTIONTYPE.Action_QueryLine);
            ...
            case R.id.btnRemove:
                if (m_reSet.getRecordCount() > 0 && !m_reSet.isEmpty()) {
                    if (m_reSet.delete()) {
                        ToastUtil.show(QueryPipeLineActivity.this, "删除线成功", 2);
                        finish();
                    } else {
                        ToastUtil.show(QueryPipeLineActivity.this, "删除线失败", 2);
                    }
                }
                try {
                    WorkSpaceUtils.getInstance().getWorkspace().save();
...
//                    _popupWindow.show();
    ...
    private void exchangePoint() {
        ...
        //排水、污水、雨水类型要换方向
        GeoLine _geoLine = (GeoLine) m_reSet.getGeometry();
//        int size = _geoLine.getPartCount();
//        LogUtills.i("线对象子数量 = ", String.valueOf(size));
        Point2Ds _point2Ds = _geoLine.getPart(0);
//        LogUtills.i("point2Ds的数量 = ", String.valueOf(_point2Ds.getCount()));
...
//        LogUtills.i("查询编辑起点埋深", df2.format(s / 100d));
            if (temp.length() > 2) {
...
//                LogUtills.i("起点埋深", "0" + df2.format(s / 100d));
    ...
    public String getEndBurialDepth() {
        ...
        if (temp.length() > 0 && !temp.equals("0.00")) {
            int s = Integer.parseInt(temp);
            DecimalFormat df2 = new DecimalFormat("####.00");
//        LogUtills.i("查询编辑终点埋深", df2.format(s / 100d));
            if (temp.length() > 2) {
//                LogUtills.i("查询编辑终点埋深", df2.format(s / 100d));
                return df2.format(s / 100d);
            } else {
//                LogUtills.i("查询编辑终点埋深", "0" + df2.format(s / 100d));
                return "0" + df2.format(s / 100d);
            }
        } else return "0";
    ...
    public String getBurialDifference() {
        ...
        if (temp.length() > 0) {
            int s = Integer.parseInt(temp);
            DecimalFormat df2 = new DecimalFormat("####.00");
//        LogUtills.i("查询编辑埋深差值", df2.format(s / 100d));

            if (temp.length() > 2) {
//                LogUtills.i("查询编辑埋深差值", df2.format(s / 100d));
                return df2.format(s / 100d);
            } else {
//                LogUtills.i("查询编辑埋深差值", "0" + df2.format(s / 100d));
                return "0" + df2.format(s / 100d);
            }
        } else
            return "0";
...
//            LogUtills.i("i起点埋深 = ", _startBurialDepth);
            ...
            int temp = (int) (s * 100);
//            LogUtills.i("i起点埋深 = ", String.valueOf(temp));
...
//            LogUtills.i("终点埋深 = ", String.valueOf(s));
            int temp = (int) (s * 100);
            String depth = String.valueOf(temp);
//            LogUtills.i("终点埋深 = ", depth);
...
//            LogUtills.i("i断面高 = ", _temp);
    ...
    public void setUsedHoleCount() {
        String _usedHoleCount = m_baseInfo.totalHole;
